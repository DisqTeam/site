...
import TableFileIcon from "../../resources/TableFileIcon";
  ...
  render() {
    return (
      <Twemoji options={{ className: "twemoji", folder: "svg", ext: ".svg" }}>
        <Head
          title="Files"
          description="Manage your files that you've uploaded to Disq."
        />
        <main>{this.state.pageState}</main>
      </Twemoji>
    );
  }
}
...
class DashboardPage extends React.Component {
  ...
  getFiles = async (page, filterChange = false) => {
    ...
    fetch(
      `${config.endpoint}/uploads/list/${page}?order=${
        this.state.order
      ...
      .then((res) => res.json())
      .then((files) => {
        if (files.files.length === 0 && filterChange)
          return this.setState({ tableData: "", maxPages: 0 });
        if (files.files.length === 0) return this.setState({ page: page - 1 });
        let tableData = files.files.map((s) => (
          <tr>
            <td>
              <span className="material-icons">{TableFileIcon(s.name)}</span>
            </td>
            <td>
              <Tippy
                theme="disq"
                ...
                content={
                  imageExtensions.includes(
                    ...
                    <img
                      src={window.location.origin + "/" + s.name}
                      width="300px"
                    ></img>
                  ) : (
                    `.${s.name.split(".")[s.name.split(".").length - 1]} file`
                  )
                }
                placement="top"
              >
                <a
                  ...
                  href={window.location.origin + "/" + s.name}
                >
                  {localStorage.censor === "true"
                    ? "[Removed]"
                    : window.location.origin + "/" + s.name}
                </a>
              </Tippy>
            </td>
            <td>
              <p>{(s.size / 1024 / 1024).toFixed(2) + "MB"}</p>
            </td>
            <td>
              <p>{dayjs.unix(s.timestamp).format("HH:mm:ss DD/MM/YYYY")}</p>
            </td>
            <td className="align_right">
              ...
              >
                <button
                  className="btn_table btn_delete btn_rod"
                  onClick={() => this.deleteFile(s.name)}
                >
      ...
      <main>
        ...
        <div className="files disq_content">
              ...
              <thead>
                <tr>
                  ...
                  <th
                    ...
                    className="url_display reactive_th"
                  >
                    URL{" "}
                    {this.state.order === "name"
                      ? this.state.order_direction === "DESC"
                        ? "▼"
                        : "▲"
                      : ""}
                  </th>
          ...
          <div className="pagination">
            <button className="btn_small" onClick={() => this.paginate(-1)}>
              Previous
            </button>
            <p>
